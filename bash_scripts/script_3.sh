...
then
  #TF_CONFIG='{"cluster": {"worker": ["83.212.80.22", "192.168.1.1"]}, "task": {"index": 0, "type": "worker"}}' python ~/distirbuted_training/python_scripts_$1.py  
  echo "Running script for $1 model on master vm and on vm 1 ,not here"
elif [ $1 == "3" ];
then
  TF_CONFIG='{"cluster": {"worker": ["83.212.80.22", "192.168.1.1", "192.168.1.3"]}, "task": {"index": 2, "type": "worker"}}' python ~/distirbuted_training/python_scripts_$1.py  
